                        ...
                        <div style="background-color: #f9f9f9; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1); position: relative;">
                            ...
                            @if($agenda->poster)
                                <div style="height: 200px; overflow: hidden;">
                                    <img src="{{ asset('storage/' . $agenda->poster) }}" alt="{{ $agenda->judul }}" style="width: 100%; height: 100%; object-fit: cover;">
                                </div>
                            @else
                            ...
                            @if(Auth::check())
                                <div style="position: absolute; top: 10px; right: 10px; display: flex; gap: 8px;">
                                    <a href="{{ route('agenda.edit', $agenda->agenda_id) }}" class="btn btn-light btn-sm" style="padding: 6px 10px; font-size: 0.85rem;">
                            ...
                            <div class="p-4">
                                ...
                                <div style="margin-bottom: 15px;">
                                    ...
                                    <p style="color: #666; margin: 5px 0;"><strong>📅 Mulai:</strong> {{ $agenda->tanggal_mulai ? $agenda->tanggal_mulai->format('d F Y H:i') : '-' }}</p>
                                    @if($agenda->tanggal_selesai)
                                        <p style="color: #666; margin: 5px 0;"><strong>📅 Selesai:</strong> {{ $agenda->tanggal_selesai->format('d F Y H:i') }}</p>
                                    @endif
