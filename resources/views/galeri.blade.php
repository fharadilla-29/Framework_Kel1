    ...
    <div class="container py-5">
                    ...
                    <div class="col-lg-6 col-xl-4 wow fadeInUp">
                        <div style="position: relative; overflow: hidden; border-radius: 8px; background-color: #f9f9f9; height: 250px;">
                            @if($gallery->medias && $gallery->medias->count() > 0)
                                ...
                                <img src="{{ asset('storage/' . $firstMedia->file_url) }}" class="w-100 h-100" style="object-fit: cover; transition: transform 0.3s ease;" alt="{{ $gallery->judul }}">
                            ...
                            @endif
                            <div style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(40, 167, 69, 0.6); opacity: 0; transition: opacity 0.3s ease; display: flex; align-items: center; justify-content: center; flex-direction: column;">
                                <div style="text-align: center;">
                                    <p style="color: white; margin: 0; font-weight: 600;">{{ $gallery->judul }}</p>
                                    <small style="color: white;">{{ $gallery->kategori }}</small>
                                    <div class="mt-3">
                                        <a href="{{ route('galeri.edit', $gallery->id) }}" class="btn btn-sm btn-light me-2" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('galeri.destroy', $gallery->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Yakin ingin menghapus galeri ini?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <!-- Media Counter -->
                        @if($gallery->medias && $gallery->medias->count() > 0)
                            <small style="color: #17a2b8;"><i class="fas fa-images me-1"></i>{{ $gallery->medias->count() }} media</small>
                        @else
                            <small style="color: #999;">{{ $gallery->created_at->format('d M Y') }}</small>
                        @endif
                        
                        <div class="mt-3 d-flex gap-2">
                            <a href="{{ route('galeri.edit', $gallery->id) }}" class="btn btn-sm text-white" style="background-color: #28a745; flex: 1; text-align: center;">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                            <form action="{{ route('galeri.destroy', $gallery->id) }}" method="POST" style="flex: 1;" onsubmit="return confirm('Yakin ingin menghapus galeri ini?');">
                                @csrf
                                @method('DELETE')
        ...
        @else
            <div class="alert alert-info text-center" role="alert">
                <i class="fas fa-info-circle"></i> Belum ada galeri. <a href="{{ route('galeri.create') }}" class="alert-link">Tambahkan sekarang</a>
            </div>
        @endif
    </div>
</div>
