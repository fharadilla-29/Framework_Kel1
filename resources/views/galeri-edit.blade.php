                    ...
                    <form action="{{ route('galeri.update', $galeri->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('kategori') is-invalid @enderror" 
                                ...
                                <option value="Kesehatan" {{ old('kategori', $galeri->kategori) == 'Kesehatan' ? 'selected' : '' }}>Kesehatan</option>
                                <option value="Pendidikan" {{ old('kategori', $galeri->kategori) == 'Pendidikan' ? 'selected' : '' }}>Pendidikan</option>
                            </select>
                            @error('kategori')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            ...
                            @if ($galeri->gambar)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $galeri->gambar) }}" alt="{{ $galeri->judul }}" 
                                         style="max-width: 100%; height: auto; border-radius: 5px; max-height: 200px;">
                                    <p class="text-muted small mt-2">Gambar saat ini</p>
                                </div>
                            @endif
                            <input type="file" class="form-control @error('gambar') is-invalid @enderror" 
                                   id="gambar" name="gambar" accept="image/*">
                            <small class="form-text text-muted">Format: JPEG, PNG, JPG, GIF. Ukuran maksimal: 2MB. Biarkan kosong jika tidak ingin mengganti gambar.</small>
                            @error('gambar')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
